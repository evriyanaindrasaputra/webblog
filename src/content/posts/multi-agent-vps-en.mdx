...
## The Model Strategy (AKA How I Pay $0)

Here's where it gets interesting. Running four agents sounds expensive, but my actual cost so far? Zero dollars on model inference.

The secret: **Google Antigravity OAuth**. It gives you free access to powerful models through a single Google account — no API keys, no credit cards, no usage limits that matter for personal use.

```yaml
models:
  - google-antigravity/claude-opus-4-6-thinking    # Free (thinking mode)
  - google-antigravity/claude-opus-4-6              # Free (non-thinking mode)
```
...
In practice, this handles 100% of my requests. Four agents, running 24/7, all on free models. The system just... works. For free.
...
**Free models are enough.** I was prepared to spend money. I didn't have to. Claude Opus 4.6 via Antigravity OAuth handles complex reasoning, multi-step tasks, and nuanced conversation without breaking a sweat. For a personal multi-agent setup, you don't need a $200/month API bill — you need one Google account.
...
2. **Use Antigravity OAuth.** One Google account gives you free access to Claude Opus 4.6 — thinking and non-thinking modes. That's enough for a multi-agent personal setup. No API keys needed.
